...
import { adminMinorStatusApi, apiError, errorMessage } from '../../services/api'
...
export default function StatiMinorePage() {
  ...
  const load = async () => {
    ...
    setError(null)
    try {
      setItems(await adminMinorStatusApi.list())
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editTarget) {
        await adminMinorStatusApi.update(editTarget.id, form)
        toast.success('Stato minore aggiornato')
      } else {
        await adminMinorStatusApi.create(form)
        toast.success('Stato minore creato')
      }
      setModalOpen(false)
      load()
    } catch (e) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await adminMinorStatusApi.delete(deleteTarget.id)
      toast.success('Stato minore eliminato')
    ...
    <>
          ...
          <Row>
            <Col xs='6'>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <h3 style={{ margin: 0 }}>Stati minore</h3>
            ...
            <Col xs='6'>
              <ol className='breadcrumb'>
                ...
                <li className='breadcrumb-item active'>Stati minore</li>
              </ol>
            </Col>
          </Row>
        </div>
                  ...
                  : (
                    <div className='table-responsive'>
                      <table className='table table-hover'>
                        ...
                        <tbody>
                          {items.length === 0
                            ? <tr><td colSpan={6} className='text-muted text-center'>Nessuno stato.</td></tr>
                            : items.map((item) => (
                              <tr key={item.id}>
      ...
      <Modal isOpen={modalOpen} toggle={() => setModalOpen(false)}>
        <ModalHeader toggle={() => setModalOpen(false)}>
          {editTarget ? 'Modifica stato' : 'Nuovo stato'}
        </ModalHeader>
        <ModalBody>
          ...
          <FormGroup check>
            ...
            />
            <Label check for='stato-active'>Attivo</Label>
          </FormGroup>
        </ModalBody>
        <ModalFooter>
      ...
      <InfoDrawer isOpen={infoOpen} onClose={() => setInfoOpen(false)} title='Guida — Stati Minore'>
        <section className='mb-4'>
          <h6 className='fw-bold mb-2'>A cosa serve</h6>
          <p style={{ fontSize: 14, color: '#444' }}>
            Definisce gli stati canonici con cui viene rappresentata la situazione gestionale
            del minore nel sistema. Lo stato del minore è un dato di processo, non un testo
            descrittivo libero.
          </p>
        </section>
        <section className='mb-4'>
          ...
          <p style={{ fontSize: 14, color: '#444' }}>
            Gli stati servono a rappresentare il ciclo gestionale del caso in modo uniforme.
            Vanno scelti da elenco canonico per consentire filtri affidabili, viste coerenti
            e processo operativo tracciabile.
          </p>
        </section>
        <section className='mb-3'>
          <h6 className='fw-bold mb-2'>Come impatta il resto del software</h6>
          <p style={{ fontSize: 14, color: '#444' }}>
            Compaiono nella lista minori, nei filtri, nei report e nel form di creazione/modifica
            del minore. Uno stato eliminato o rinominato impatta tutti i minori che lo utilizzano.
          </p>
        </section>
      </InfoDrawer>
    </>
