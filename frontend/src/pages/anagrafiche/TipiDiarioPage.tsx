...
import { adminJournalEntryTypeApi, apiError } from '../../services/api'
import type { JournalEntryType } from '../../types'

const EMPTY_FORM = { code: '', name: '', description: '', sort_order: '', is_active: true }

export default function TipiDiarioPage() {
  const [items, setItems]       = useState<JournalEntryType[]>([])
  const [loading, setLoading]   = useState(true)
  const [error, setError]       = useState<string | null>(null)

  const [modalOpen, setModalOpen]     = useState(false)
  const [editTarget, setEditTarget]   = useState<JournalEntryType | null>(null)
  const [form, setForm]               = useState(EMPTY_FORM)
  const [fieldErrors, setFieldErrors] = useState<Record<string, string[]>>({})
  const [saving, setSaving]           = useState(false)
  const [conflictMsg, setConflictMsg] = useState<string | null>(null)

  const [deleteTarget, setDeleteTarget]     = useState<JournalEntryType | null>(null)
  const [deleteConflict, setDeleteConflict] = useState<string | null>(null)
  const [deleting, setDeleting]             = useState(false)

  const load = async () => {
    setLoading(true); setError(null)
    try { setItems(await adminJournalEntryTypeApi.list()) }
  ...
  const openEdit = (item: JournalEntryType) => {
    setEditTarget(item)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editTarget) { await adminJournalEntryTypeApi.update(editTarget.id, payload as Omit<JournalEntryType, 'id'>); toast.success('Tipo aggiornato') }
      else { await adminJournalEntryTypeApi.create(payload as Omit<JournalEntryType, 'id'>); toast.success('Tipo creato') }
  ...
  const handleDelete = async () => {
    ...
    try {
      await adminJournalEntryTypeApi.delete(deleteTarget.id)
      toast.success('Tipo eliminato'); setDeleteTarget(null); load()
    } catch (e) {
      const ae = apiError(e)
      if (ae.status === 409) setDeleteConflict(ae.message ?? 'Tipo in uso da voci diario esistenti.')
  ...
  return (
    <Container fluid>
      ...
      <Modal isOpen={modalOpen} toggle={() => setModalOpen(false)} size='md'>
        <ModalHeader toggle={() => setModalOpen(false)}>{editTarget ? 'Modifica tipo' : 'Nuovo tipo voce diario'}</ModalHeader>
        <ModalBody>
          {conflictMsg && <Alert color='warning'>{conflictMsg}</Alert>}
          <FormGroup>
            <Label>Codice <span className='text-danger'>*</span></Label>
            <Input value={form.code} onChange={(e) => setForm((p) => ({ ...p, code: e.target.value.toUpperCase() }))} placeholder='Es. OBSERVATION' invalid={!!fErr('code')} disabled={!!editTarget} />
          ...
          <FormGroup>
            ...
            <Input value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} placeholder='Es. Osservazione comportamentale' invalid={!!fErr('name')} />
            ...
            <Col md={6}>
              <FormGroup className='mt-4 pt-1'>
                <div className='d-flex align-items-center gap-2'>
                  <Input type='checkbox' id='is_active_tipodiario' checked={form.is_active} onChange={(e) => setForm((p) => ({ ...p, is_active: e.target.checked }))} style={{ width: 16, height: 16 }} />
                  <Label for='is_active_tipodiario' className='mb-0'>Tipo attivo</Label>
                </div>
              </FormGroup>
            </Col>
          </Row>
