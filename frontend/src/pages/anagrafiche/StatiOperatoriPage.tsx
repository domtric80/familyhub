...
import { adminStaffStatusApi, apiError } from '../../services/api'
...
export default function StatiOperatoriPage() {
  const [items, setItems]       = useState<StaffStatus[]>([])
  const [loading, setLoading]   = useState(true)
  const [error, setError]       = useState<string | null>(null)

  const [modalOpen, setModalOpen]     = useState(false)
  const [editTarget, setEditTarget]   = useState<StaffStatus | null>(null)
  const [form, setForm]               = useState(EMPTY_FORM)
  const [fieldErrors, setFieldErrors] = useState<Record<string, string[]>>({})
  const [saving, setSaving]           = useState(false)
  const [conflictMsg, setConflictMsg] = useState<string | null>(null)

  const [deleteTarget, setDeleteTarget]     = useState<StaffStatus | null>(null)
  ...
  const load = async () => {
    ...
    setError(null)
    try {
      setItems(await adminStaffStatusApi.list())
    } catch (e) {
      setError(apiError(e).message ?? 'Errore caricamento stati operatore')
    } finally {
      setLoading(false)
    }
  }
  ...
  const openEdit = (item: StaffStatus) => {
    setEditTarget(item)
    setForm({
      code:        item.code,
      name:        item.name,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editTarget) {
        await adminStaffStatusApi.update(editTarget.id, payload)
        toast.success('Stato aggiornato')
      } else {
        await adminStaffStatusApi.create(payload as Omit<StaffStatus, 'id'>)
  ...
  const handleDelete = async () => {
    ...
    try {
      await adminStaffStatusApi.delete(deleteTarget.id)
      ...
      const ae = apiError(e)
      if (ae.status === 409) {
        setDeleteConflict(ae.message ?? 'Stato in uso da operatori esistenti.')
      } else {
    ...
    <Container fluid>
      ...
      <Modal isOpen={modalOpen} toggle={() => setModalOpen(false)} size='md'>
        <ModalHeader toggle={() => setModalOpen(false)}>
          {editTarget ? 'Modifica stato' : 'Nuovo stato operatore'}
        </ModalHeader>
        <ModalBody>
            ...
            <Col md={6}>
              <FormGroup className='mt-4 pt-1'>
                <div className='d-flex align-items-center gap-2'>
                  ...
                  />
                  <Label for='is_active_stato' className='mb-0'>Stato attivo</Label>
                </div>
              </FormGroup>
            </Col>
          </Row>
      ...
      <Modal isOpen={!!deleteTarget} toggle={() => setDeleteTarget(null)}>
        <ModalHeader toggle={() => setDeleteTarget(null)}>Elimina stato</ModalHeader>
        <ModalBody>
          {deleteConflict
            ? <Alert color='warning'>{deleteConflict} Non è possibile eliminare uno stato assegnato a operatori esistenti.</Alert>
