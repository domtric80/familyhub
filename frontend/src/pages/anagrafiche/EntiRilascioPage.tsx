...
import { adminDocumentIssuerApi, apiError } from '../../services/api'
import type { DocumentIssuer } from '../../types'

const EMPTY_FORM = { code: '', name: '', description: '', sort_order: '', is_active: true }

export default function EntiRilascioPage() {
  const [items, setItems]       = useState<DocumentIssuer[]>([])
  const [loading, setLoading]   = useState(true)
  const [error, setError]       = useState<string | null>(null)
  const [modalOpen, setModalOpen]     = useState(false)
  const [editTarget, setEditTarget]   = useState<DocumentIssuer | null>(null)
  const [form, setForm]               = useState(EMPTY_FORM)
  const [fieldErrors, setFieldErrors] = useState<Record<string, string[]>>({})
  const [saving, setSaving]           = useState(false)
  const [conflictMsg, setConflictMsg] = useState<string | null>(null)
  const [deleteTarget, setDeleteTarget]     = useState<DocumentIssuer | null>(null)
  const [deleteConflict, setDeleteConflict] = useState<string | null>(null)
  const [deleting, setDeleting]             = useState(false)

  const load = async () => {
    setLoading(true); setError(null)
    try { setItems(await adminDocumentIssuerApi.list()) }
  ...
  const openEdit = (item: DocumentIssuer) => {
    setEditTarget(item)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editTarget) { await adminDocumentIssuerApi.update(editTarget.id, payload); toast.success('Ente aggiornato') }
      else { await adminDocumentIssuerApi.create(payload as Omit<DocumentIssuer, 'id'>); toast.success('Ente creato') }
  ...
  const handleDelete = async () => {
    ...
    try { await adminDocumentIssuerApi.delete(deleteTarget.id); toast.success('Ente eliminato'); setDeleteTarget(null); load() }
    catch (e) {
      const ae = apiError(e)
      if (ae.status === 409) setDeleteConflict(ae.message ?? 'Ente in uso da documenti esistenti.')
  ...
  return (
    <Container fluid>
      ...
      <Row><Col xs={12}>
        <Card>
          <CardHeader className='d-flex justify-content-between align-items-center'>
            <h5 className='mb-0'>Elenco enti</h5>
            <Button color='primary' size='sm' className='d-flex align-items-center gap-1' onClick={openCreate}><Plus size={14} /> Nuovo ente</Button>
          </CardHeader>
          <CardBody>
            ...
            {loading ? <div className='text-center py-5'><div className='loader' /></div> : (
              <div className='table-responsive'>
                <table className='table table-hover'>
                  ...
                  <tbody>
                    {items.length === 0 && <tr><td colSpan={6} className='text-center text-muted py-4'>Nessun ente configurato.</td></tr>}
                    {items.map((item) => (
                      <tr key={item.id}>
      ...
      <Modal isOpen={modalOpen} toggle={() => setModalOpen(false)} size='md'>
        <ModalHeader toggle={() => setModalOpen(false)}>{editTarget ? 'Modifica ente' : 'Nuovo ente rilascio'}</ModalHeader>
        <ModalBody>
          {conflictMsg && <Alert color='warning'>{conflictMsg}</Alert>}
          <FormGroup>
            <Label>Codice <span className='text-danger'>*</span></Label>
            <Input value={form.code} onChange={(e) => setForm((p) => ({ ...p, code: e.target.value.toUpperCase() }))} placeholder='Es. COMUNE' invalid={!!fErr('code')} disabled={!!editTarget} />
          ...
          <FormGroup>
            ...
            <Input value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} placeholder='Es. Comune' invalid={!!fErr('name')} />
            ...
            <Col md={6}>
              <FormGroup className='mt-4 pt-1'>
                <div className='d-flex align-items-center gap-2'>
                  <Input type='checkbox' id='is_active_ente' checked={form.is_active} onChange={(e) => setForm((p) => ({ ...p, is_active: e.target.checked }))} style={{ width: 16, height: 16 }} />
                  <Label for='is_active_ente' className='mb-0'>Stato attivo</Label>
                </div>
              </FormGroup>
            </Col>
          </Row>
        ...
        <ModalFooter>
          <Button color='primary' onClick={handleSave} disabled={saving}>{saving ? 'Salvataggio…' : editTarget ? 'Salva modifiche' : 'Crea ente'}</Button>
          <Button color='light' onClick={() => setModalOpen(false)}>Annulla</Button>
        </ModalFooter>
      </Modal>

      <Modal isOpen={!!deleteTarget} toggle={() => setDeleteTarget(null)}>
        <ModalHeader toggle={() => setDeleteTarget(null)}>Elimina ente rilascio</ModalHeader>
        <ModalBody>
          {deleteConflict
            ? <Alert color='warning'>{deleteConflict} Non è possibile eliminare un ente usato in documenti esistenti.</Alert>
            : <p>Eliminare l'ente <strong>{deleteTarget?.name}</strong> (<code>{deleteTarget?.code}</code>)?</p>}
        </ModalBody>
        <ModalFooter>
